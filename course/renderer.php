...
class core_course_renderer extends plugin_renderer_base {
    ...
    public function course_modchooser($modules, $course) {
        ...
        // Put all options into one tag 'modoptions' to allow us to handle scrolling
        $formcontent .= html_writer::start_tag('div', array('class' => 'modoptions'));
        ...
        $formcontent .= html_writer::tag('input', '',
                array('type' => 'submit', 'name' => 'submit', 'id' => 'submit', 'value' => get_string('next')));
        $formcontent .= html_writer::tag('input', '',
        ...
        $formcontent = html_writer::tag('div', $formcontent, array('id' => 'choosemod'));

        // Put all of the content together
        $content = $formcontent;

        $content = html_writer::tag('div', $content, array('id' => 'modchoosercontainer'));
        return $header . html_writer::tag('div', $content, array('id' => 'modchooserdialogue'));
    }
        ...
        foreach ($modules as $module) {
            if (!isset($module->types)) {
                ...
                foreach ($module->types as $type) {
                    $return .= $this->course_modchooser_module($type, array('subtype'));
                }
            }
        }
        return $return;
     ...
     * @return string The composed HTML for the module
     */
    protected function course_modchooser_module($module, $classes = array()) {
        $output = '';
        $classes[] = 'modoption';
