...
/**
 * Attach handler to url_selects
 */
M.util.init_url_selects = function(Y) {
    YUI().use('node', function(Y) {
        Y.delegate('change', function(e) {
            e = e || window.event;
            var element = Y.one(e.target);
            if (element.get('value') != false) {
                window.location = M.cfg.wwwroot+element.get('value');
            }
        },
        '#page-content', '.urlselect select.menujump');
    });
};
